...
export class PortfolioComponent {
  ...
  ngOnInit(): void {
    ...
    var total = newArray.reduce(function (acc, obj) { return acc + Math.abs(obj.percent); }, 0);
    newArray.forEach(function (obj) { obj.percent = obj.percent / total * (scale); });
    newArray.forEach(function (obj) { obj.percent = Math.max(obj.percent, 0); });
    ...
    var total = newArray.reduce(function (acc, obj) { return acc + Math.abs(obj.percent); }, 0);
    newArray.forEach(function (obj) { obj.percent = obj.percent / total * 100; });
